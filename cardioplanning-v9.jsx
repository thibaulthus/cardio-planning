...
/* v10.133 : le pointillé se règle dans Paramètres (carte 🎯) — couleur et transparence.
...
function gvGauche(){var rows=document.querySelectorAll("[data-day]");if(!rows.length)return null;var el=gvCadre(rows[0]);if(!el)return null;var av={el:el,left:el.scrollLeft,top:el.scrollTop};try{el.scrollTo({left:0,behavior:"smooth"});}catch(e){el.scrollLeft=0;}return av;}
/* v10.145 : au retrait de la colonne, retour à la position d'avant — seulement si rien n'a bougé entre-temps
   (cadre toujours tout à gauche, même hauteur, même onglet). Un défilement, un autre onglet : on ne bouge pas. */
function gvRetour(av){if(!av||!av.el||!document.contains(av.el))return;var el=av.el;if(el.scrollLeft>1||Math.abs(el.scrollTop-av.top)>2||av.left<=0)return;try{el.scrollTo({left:av.left,behavior:"smooth"});}catch(e){el.scrollLeft=av.left;}}
...
function TableScroll({children,style,mh=150,jours=false,memId=null,fit=false,memX=null,centre=null}){
  ...
  const doFit=React.useCallback(()=>{
    ...
    /* v10.47, ses deux retours de test :
       ...
       valeur d'origine. */
    /* v10.132 : le bandeau fixé en bas (hors-ligne, PIN médecin…) mordait sur la dernière ligne
...
function InternesTile({intCfg,setIntCfg,actes=[],pins=[]}){
  ...
  return <div style={{...S.card,marginBottom:10}}>
    <div style={{fontWeight:700,color:"#388bfd",fontSize:13,marginBottom:8}}>🎓 Internes</div>
...
function InternesView({intCfg,setIntCfg=null,actes,acteById,getEntries,setEntry,isVac,year,month,allDays,viewPeriod,showFull,setShowFull,canEdit,canSalle=false,salleReg=[],intSelf=false,prevM,nextM,darkMode,setDarkMode}){
  ...
  return <div>
    <div style={{...S.bar,position:"sticky",top:HDR_H,zIndex:40,background:"var(--bg)",paddingTop:6,paddingBottom:6}}>
      ...
      <div style={{display:"flex",gap:6,alignItems:"center",marginLeft:"auto"}}>
        {/* v10.133 : la pastille « date → date » est retirée — les dates sont dans la grille */}
...
function CardioPlanning(){
  ...
  const intGardeMem=useRef(null);   /* v10.145 : position du cadre avant Garde int. */
...
`}</style>
      ...
      {tab==="partage"&&accessMode!=="adminEdit"&&!isMedEdit&&(
        <div style={{maxWidth:500}} className={"pset "+psetFold.map(i=>"pf"+i).join(" ")} ref={psetRef} onClick={psetClick}>
            ...
            <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
              {medecins.filter(m=>(m.role||"medecin")==="medecin"||m.role==="attache").map(m=>{const on=(colSelf.off||[]).map(String).indexOf(String(m.id))<0;
                return <button key={m.id} onClick={()=>setColSelf(c=>{const l=((c||{}).off||[]).map(String);const i2=l.indexOf(String(m.id));if(i2>=0)l.splice(i2,1);else l.push(String(m.id));return {...(c||{}),off:l};})}
                  style={{fontSize:11,padding:"3px 11px",borderRadius:6,fontWeight:700,cursor:"pointer",border:on?"2px dashed "+lisRgba(colSelf.col,colSelf.op):"1px solid var(--border)",background:on?lisRgba(colSelf.col,.10):"var(--bg3)",color:on?lisRgba(colSelf.col,1):"var(--txt3)"}}>{m.init}</button>;})}
            </div>
          </div>}
          {isEdit&&<div style={{...S.card,marginBottom:10}}>
          ...
          <div style={{...S.card,marginBottom:10}}>{/* v10.137 : Salles, carte à part */}
