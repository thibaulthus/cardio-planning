...
const APP_VERSION="v9.4 — 18/07/2026";
...
const HELP_SECTIONS=[
 ...
 {id:"onglets",icon:"📑",title:"Les onglets un par un",body:()=>HE("div",null,
  ...
  HTab({t:"📥 Reports",children:["outil individuel et facultatif : cochez vos semaines blanches, l'application propose le décalage de vos semaines à reporter (tour, absences) vers vos semaines blanches aux dates habituelles, signale ce qui est sans report possible par consultation, et liste les places libres. Export CSV."]}))},
...
function ReportsView(p){
  ...
  const analysis=React.useMemo(()=>{
    ...
    /* Semaines blanches d'accueil : sans tour, sans perte, au moins une date habituelle disponible */
    ...
    /* Appariement semaine → semaine blanche : toutes les dates perdues doivent y être disponibles */
    const usedW=new Set();
    ...
    /* Sans report possible : regroupé par activité */
    const noRepByActe={};
    weekPairs.filter(wp=>!wp.to).forEach(wp=>wp.lost.forEach(L=>{(noRepByActe[L.acte]=noRepByActe[L.acte]||[]).push(L);}));
    /* Places libres restantes (hors dates déjà proposées en destination) */
    const usedKeys=new Set();weekPairs.forEach(wp=>{if(wp.dest)wp.dest.forEach(D=>usedKeys.add(dk3(D.y,D.m,D.d)+D.sl));});
    const restHab=[],offHab=[],offAutre=[];
    days.forEach(o=>{
      const dt=new Date(o.y,o.m,o.d),dw=dt.getDay();
      if(dw===0||dw===6||isFerie(o.y,o.m,o.d))return;
      ...
      ["M","AM"].forEach(sl=>{
        ...
        else if(!blanche&&!hab&&slotState(o.y,o.m,o.d,sl)==="free"){(habDW.indexOf(dw)>=0?offHab:offAutre).push({y:o.y,m:o.m,d:o.d,sl});}
      });
    });
    const tourWeeks=[];weeks.forEach(w=>{const wm=tourMed[w.key]||{};
      if((wm.HC||[]).includes(mid)||(wm.USIC||[]).includes(mid)){const nb=w.days.filter(o=>isBl(o.y,o.m,o.d)).length;tourWeeks.push({key:w.key,days:w.days,nbBl:nb});}});
    return {weekPairs,noRepByActe,restHab,offHab,offAutre,tourWeeks};
  ...
  return RE("div",{style:{maxWidth:820}},
    ...
    /* ── Rapport ── */
    RE("div",{style:{fontSize:10,fontWeight:800,color:"var(--txt2)",textTransform:"uppercase",letterSpacing:.4,marginBottom:6}},"Propositions de report"),
    analysis.tourWeeks.length>0&&RE("div",{style:{fontSize:11,color:"var(--txt)",marginBottom:8,lineHeight:1.7}},
      RE("b",null,"Tour ↔ blanches : "),
      analysis.tourWeeks.map(t=>RE("span",{key:t.key,style:{display:"inline-block",marginRight:8,padding:"1px 7px",borderRadius:6,fontSize:10,fontWeight:700,
        background:t.nbBl>=5?"rgba(22,163,74,.15)":t.nbBl>0?"rgba(245,158,11,.15)":"rgba(239,68,68,.12)",
        color:t.nbBl>=5?"#16a34a":t.nbBl>0?"#b45309":"#ef4444"}},
        "sem. "+t.days[0].d+" "+MOIS[t.days[0].m].slice(0,4)+" : "+(t.nbBl>=5?"✓ blanche":t.nbBl>0?t.nbBl+"/5 blanche":"non blanche")))),
    analysis.weekPairs.length===0
      ?RE("div",{style:{fontSize:12,color:"#16a34a",fontWeight:700,marginBottom:10}},"✓ Aucune consultation à reporter sur la période.")
      :RE("div",{style:{border:"1px solid var(--border)",borderRadius:8,overflow:"hidden",marginBottom:10}},
        analysis.weekPairs.map((wp,i)=>{
          const f=wp.lost[0];const isT=wp.lost.some(L=>L.why==="tour");
          return RE("div",{key:wp.wk,style:{padding:"7px 10px",borderTop:i>0?"1px solid var(--border2)":"none",fontSize:11}},
            RE("div",{style:{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:3}},
              RE("span",{style:{fontWeight:800,color:"var(--txt)"}},"Semaine du "+f.d+" "+MOIS[f.m].slice(0,4)),
              RE("span",{style:{fontSize:9,padding:"1px 6px",borderRadius:5,fontWeight:800,background:isT?"rgba(29,78,216,.12)":"rgba(239,68,68,.12)",color:isT?"#1d4ed8":"#ef4444"}},isT?"tour":"absence"),
              RE("span",{style:{color:"var(--txt3)"}},"→"),
              wp.to
                ?RE("span",{style:{fontWeight:800,color:"#16a34a"}},"semaine blanche du "+wp.to.days[0].d+" "+MOIS[wp.to.days[0].m].slice(0,4))
                :RE("span",{style:{color:"#ef4444",fontWeight:800}},"pas de semaine blanche disponible")),
            wp.to&&RE("div",{style:{fontSize:10,color:"var(--txt2)",lineHeight:1.7}},
              wp.lost.map((L,k)=>{const a2=acteOf(L.acte);
                return RE("span",{key:k,style:{marginRight:10,display:"inline-flex",alignItems:"center",gap:4}},
                  a2&&RE("span",{style:{padding:"0 5px",borderRadius:4,fontSize:8,fontWeight:800,fontFamily:"'JetBrains Mono',monospace",background:a2.color,color:"#111"}},a2.short),
                  fmtD(L)+" "+L.sl+" → "+fmtD(wp.dest[k])+" "+wp.dest[k].sl);})));
        })),
    Object.keys(analysis.noRepByActe).length>0&&RE("div",{style:{marginBottom:10}},
      RE("div",{style:{fontSize:10,fontWeight:800,color:"#ef4444",textTransform:"uppercase",letterSpacing:.4,marginBottom:4}},"Sans report possible — par consultation"),
      Object.keys(analysis.noRepByActe).map(aid=>{const a2=acteOf(aid),list2=analysis.noRepByActe[aid];
        return RE("div",{key:aid,style:{fontSize:11,color:"var(--txt)",marginBottom:3,display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}},
          RE("span",{style:{padding:"1px 6px",borderRadius:4,fontSize:9,fontWeight:800,fontFamily:"'JetBrains Mono',monospace",background:a2?a2.color:"#ccc",color:"#111"}},a2?a2.short:aid),
          RE("span",{style:{color:"var(--txt2)"}},list2.map(L=>fmtD(L)+" "+L.sl).join(" · ")));})),
    RE("div",{style:{fontSize:11,color:"var(--txt)",lineHeight:1.7,marginBottom:10}},
      RE("b",null,"Places libres restantes : "),
      (analysis.restHab.length+analysis.offHab.length+analysis.offAutre.length)===0&&RE("span",{style:{color:"#b45309",fontWeight:700}},"aucune — pensez aux créneaux de 3-4 patients pendant vos semaines de tour."),
      analysis.restHab.length>0&&RE("div",{style:{fontSize:10,color:"var(--txt2)"}},RE("b",{style:{color:"#b45309"}},"Dates habituelles en semaine blanche (à réouvrir en priorité) : "),analysis.restHab.slice(0,16).map(o=>fmtD(o)+" "+o.sl).join(" · ")+(analysis.restHab.length>16?" …":"")),
      analysis.offHab.length>0&&RE("div",{style:{fontSize:10,color:"var(--txt2)"}},RE("b",{style:{color:"#7c3aed"}},"Offs sur vos jours de consultation : "),analysis.offHab.slice(0,16).map(o=>fmtD(o)+" "+o.sl).join(" · ")+(analysis.offHab.length>16?" …":"")),
      analysis.offAutre.length>0&&RE("div",{style:{fontSize:10,color:"var(--txt3)"}},RE("b",null,"Autres offs : "),analysis.offAutre.slice(0,16).map(o=>fmtD(o)+" "+o.sl).join(" · ")+(analysis.offAutre.length>16?" …":""))),
...
function CardioPlanning(){
  ...
  const DEFAULT_TABS=[["planning","📅 Planning"],["tourmedical","🔄 Tour"],["chl","🏥 CHL"],["chb","🏥 CHB"],["plateau","❤️ PT Cardio"],["angio","🔬 PT Angio"],["garde","🌙 Gardes"],["astreinte","📞 Astreinte"],["plantype","📋 Type"],["attache","👔 Attachés"],["activites","⚙️ Activités"],["equipe","👥 Équipe"],["partage","⚙️ Paramètres"],["stats","📊 Stats"],["reports","📥 Reports"],["aide","❓ Aide"]];
